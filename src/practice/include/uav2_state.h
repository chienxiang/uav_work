...
class uav_state
{
    public:
        ...
        ros::Subscriber local_pose_uav0_sub;
        ros::Subscriber local_vec_uav0_sub;
        ros::Subscriber local_pos_uav1_sub;
        ros::Subscriber local_vec_uav1_sub;
        ...
        int well,mode;
        float px,py,pz,vx,vy,vz,ax,ay,az,yaw_t,g_x=5,g_y=5,g_z=2,g_rad,avoid[3],distent,uav0_vx,uav0_vy,uav0_vz,uav0_px,uav0_py,uav0_pz,uav1_vx,uav1_vy,uav1_vz,uav1_px,uav1_py,uav1_pz,start=1,landing=0;

        uav_state(){
            local_pos_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav2/mavros/local_position/pose", 10,&uav_state::subpose,this);

            local_vec_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav2/pidvec", 10,&uav_state::subvec,this);

            local_yaw_sub = nh.subscribe<mavros_msgs::PositionTarget>("/uav2/mavros/setpoint_raw/target_local", 10, &uav_state::subyaw, this);

            local_vec_pub =  nh.advertise<geometry_msgs::TwistStamped>("/uav2/pidvec", 10);

            state_sub = nh.subscribe<mavros_msgs::State>("/uav2/mavros/state", 10,&uav_state::state_cb,this );
	   
            obstacle_sub = nh.subscribe<uav_path::avoid_vector>("/uav2/avoid_vector", 10,&uav_state::obstacle,this);

            target = nh.subscribe<uav_path::formation>("/uav2/target", 10, &uav_state::subtarget,this);

            land = nh.subscribe<uav_path::land>("/uav2/land", 10, &uav_state::subland,this);

            scale = nh.advertise<uav_path::scale>("/uav2/scale", 10);

            local_pose_uav0_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav0/mavros/local_position/pose", 10, &uav_state::subpose_uav0,this);

            local_vec_uav0_sub = nh.subscribe<geometry_msgs::TwistStamped>("uav0/pidvec", 10, &uav_state::subvec_uav0,this);

            local_pos_uav1_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav1/mavros/local_position/pose", 10, &uav_state::subpose_uav1,this);

            local_vec_uav1_sub = nh.subscribe<geometry_msgs::TwistStamped>("uav1/pidvec", 10, &uav_state::subvec_uav1,this);  

            local_pos_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav2/mavros/setpoint_raw/local", 10);
	...
	void subvec_uav1(const geometry_msgs::TwistStamped::ConstPtr &msg){
   	uav1_vx=msg->twist.linear.x; uav1_vy=msg->twist.linear.y; uav1_vz=msg->twist.linear.z;      }

	void subpose_uav1(const geometry_msgs::PoseStamped::ConstPtr &msg){
   	uav1_px=msg->pose.position.x+1; uav1_py=msg->pose.position.y; uav1_pz=msg->pose.position.z;   }
...
void *qr1(float atf[],float cx,float cy,float cz){
    ...
    float size2=sqrt(pow(uav1_vx,2)+pow(uav1_vy,2)+pow(uav1_vz,2));
    atf[0]=(cz/size1)*(uav1_vy/size2)-(cy/size1)*(uav1_vz/size2); atf[1]=(cx/size1)*(uav1_vz/size2)-(cz/size1*uav1_vx/size2); atf[2]=(cy/size1)*(uav1_vx/size2)-(cx/size1)*(uav1_vy/size2);
